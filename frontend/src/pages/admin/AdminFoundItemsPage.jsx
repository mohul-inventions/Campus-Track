...
import { foundAPI, adminAPI } from '../../services/api';
import { useToast } from '../../contexts/ToastContext';
import StatusBadge from '../../components/StatusBadge';
import { Compass, Search, Edit2, X } from 'lucide-react';
...
export default function AdminFoundItemsPage() {
  ...
  const [newStatus, setNewStatus] = useState('Found');
  const { success, error } = useToast();

  const fetchItems = async () => {
    try {
      setLoading(true);
      const res = await foundAPI.getAll({ search });
  ...
  const handleUpdateStatus = async (e) => {
    e.preventDefault();
    try {
      const res = await adminAPI.updateItemStatus('found', editingItem.found_id, newStatus, 'Admin override');
      if (res.success) {
        success(res.message);
        setEditingItem(null);
        fetchItems();
  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="glass-panel rounded-3xl p-6 sm:p-8 border border-slate-800/80 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-100">Found Items Custody Directory</h1>
          <p className="text-xs text-slate-400 mt-1">Track physical custody locations and update registry states</p>
        </div>
        <div className="relative w-full sm:w-64">
          <Search className="w-4 h-4 text-slate-400 absolute left-3.5 top-1/2 -translate-y-1/2" />
          <input
            type="text"
            placeholder="Search found items, brand..."
            value={search}
          ...
          <table className="w-full text-left text-xs text-slate-300">
            <thead className="bg-dark-900/80 text-[11px] font-mono text-gold-400 uppercase tracking-wider border-b border-slate-800">
              <tr>
                ...
                <th className="py-3 px-4">Found At</th>
                <th className="py-3 px-4">Current Custody Desk</th>
            ...
            <tbody className="divide-y divide-slate-800/60">
              {items.map(i => (
                <tr key={i.found_id} className="hover:bg-slate-800/30 transition-colors">
                  <td className="py-3 px-4 font-mono font-bold text-slate-400">#F-{i.found_id}</td>
                  ...
                  <td className="py-3 px-4 text-emerald-400 font-semibold">{i.storage_location}</td>
                  <td className="py-3 px-4 font-mono text-slate-400">{i.date_found?.split('T')[0]}</td>
